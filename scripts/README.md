...
プロキシを per-request 指定する手段がないため、bookmarklet (Memola) から
...
```
Memola (browser)  ─── fetch ───>  http://localhost:18080  ─┐
                                                          │
                                  +--- requests + proxies +
                                  ▼
                       (HTTPS via オンプレプロキシ)
...
────────────────────────────────────────────────────────────────
Memola の設定モーダルに「ベース URL」を入力 (どちらでも可):
...
あるいは Memola を起動して AI パネルで何か質問。バッジが
「企業AI · gpt-4.1-mini」に変わっていれば設定 OK。
